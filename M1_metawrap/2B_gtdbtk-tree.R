...
# read in full tree
fulltree<-phytools::read.newick("M1_mag.bac120.classify.clean.tree")
tips <- fulltree$tip.label

# read in full taxonomy
gtdb.taxa<-read.delim("M1_mag/bac120_taxonomy_r202.tsv",header = F) %>% separate(col = V2,sep = ";",into = c("Domain","Phylum","Class","Order","Family","Genus","Species"))

# read in bin taxa
bin.taxa<-read.delim("M1_mag-refs-taxaids.txt",
...
classif <- read.delim("M1_mag.bac120.summary.tsv", na.strings = '') %>% separate(col = classification,sep = ";",into = c("Domain","Phylum","Class","Order","Family","Genus","Species"))

############
subtree<-phytools::read.newick("M1_mag.bac120.classify.clean.sub.tre")
