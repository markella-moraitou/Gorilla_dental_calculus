...
do
    ...
    #Check if either the accession number ($1) or the corresponding RefSeq accession number ($18) match
    awk -vLOOKUPVAL=$i -v OFS='\t' '
    BEGIN {FS="\t"}; $1 == LOOKUPVAL || $18 == LOOKUPVAL {print LOOKUPVAL, $8}' < $summary_file >> species_names.txt &
    #Wait once more than 5 jobs are running
    if [[ $(jobs -p | grep "^[0-9]" | wc -l) > 5 ]]
    then
        wait
    fi
done
