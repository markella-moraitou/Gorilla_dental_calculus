...
GO_BP_phyloseq<-readRDS("F2_functional_stats/data/GO_BP_phyloseq") %>% 
...
# w = 0.8
ancom_res<-read_csv("F2_functional_stats/data/BP_ancom-read-depth.csv") %>% 
  filter(detected_0.9==TRUE & !is.infinite(W))

# stratified GO terms
humann2_pathways <- read_tsv("F2_functional_stats/data/all_GOterms_cpm_renamed_wspecies.tsv",col_names = T) %>% 
...
ggsave(plot = comb_p,filename = paste0("F2_functional_stats/top.sp_ancom_0.9.png"),dpi = 300,width = 16,height = 16,units = "in")
